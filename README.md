...
# 코드 참조 및 개선
본 프로젝트의 일부 코드는 aiexeauto 레포지토리를 참조하여, 다음과 같은 부분을 개선하였습니다.

구조화된 코드 패턴
원본 코드를 바탕으로 모듈화를 진행해 유지보수성과 확장성을 높였습니다.
Docker 연동 최적화
Docker 컨테이너 생성 및 종료 로직을 간소화하여 운영에 필요한 리소스를 최소화했습니다.
에러 처리 강화
예상치 못한 예외 상황(파일 접근 오류, 경로 오류 등)을 처리하는 루틴을 보강했습니다.
UI/UX 개선
콘솔 스피너, 컬러 출력, 박스 스타일(boxen) 등을 활용해 진행 상황을 직관적으로 확인할 수 있도록 변경했습니다.
위 과정에서 원본 프로젝트의 아이디어와 코드 일부를 발췌해 사용했으며, 자체적으로 재구성하고 확장함으로써 보다 완성도 높은 자동화 환경을 구현했습니다. 원본 프로젝트에 대한 자세한 정보는 aiexeauto GitHub 저장소에서 확인하실 수 있습니다.
